...
static int reconfig(struct vo *vo, struct mp_image_params *params, int flags)
{
    ...
    vo_wayland_config(vo, vo->dwidth, vo->dheight,
                            flags);
...
static bool render_to_screen(struct priv *p, struct mp_image *mpi)
{
    ...
    status = vaGetSurfaceBufferWl(p->display, surface, flags, &buffer);
    CHECK_VA_STATUS(p, "vaGetSurfaceBufferWl()");

    wl_surface_attach(p->vo->wayland->window.surface, buffer, 0, 0);
    wl_surface_damage(p->vo->wayland->window.surface, 0, 0,
    ...
    wl_surface_commit(p->vo->wayland->window.surface);
...
const struct vo_driver video_out_vaapi_wayland = {
    .description = "VA API with wayland",
    .name = "vaapi-wayland",
    .preinit = preinit,
